...
const MONGODB_URI = 'mongodb+srv://fullstack:<password>@cluster0.e949m.mongodb.net/book?retryWrites=true&w=majority'

console.log('connecting to', MONGODB_URI)

mongoose.connect(MONGODB_URI).then(() => console.log('connected to MongoDB')).catch(error => console.log('error connection to MongoDB: ', error.message))
...
const server = new ApolloServer({
    ...
    context: async({req}) => {
        ...
        if(auth && auth.toLowerCase().startsWith('bearer ')){
            const decodedToken = jwt.verity(auth.substring(7), JWT_SECRET)
